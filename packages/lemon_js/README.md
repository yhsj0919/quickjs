...
`lemon_js` 是面向 Flutter 的 QuickJS 运行时，覆盖原生 FFI 与 Flutter Web
WASM，并提供模块、插件、宿主能力注入和结构化值转换。
...
`../../examples/lemon_js_example/build/windows` 中查找已有 `quickjs.dll`；未找到时自动执行一次
`flutter build windows --debug`。随后运行 `packages/lemon_js_ui` 的静态分析和
...
- `lib/lemon_js.dart`：公开 package 入口。
...
- `../../examples/lemon_js_example/`：workspace 中的手动 smoke test app 与示例页面。
