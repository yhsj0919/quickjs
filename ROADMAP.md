...
- [x] example 已覆盖 basic eval、async API、runtime worker、queue/reentry、
  runtime isolation、exception model、resource limit、structured values、callback bridge 页面入口；其中
  exception model 页面已覆盖基础错误类型与 `JsException` 结构化字段展示。
...
- [~] Promise-based Dart function 注入：`engine.bind(...)`，JS 侧调用返回 Promise；
  native 最小闭环已实现，Web 对齐待补。
- [~] Dart callback 参数映射：native 已覆盖 JSON-compatible primitives、array、plain object。
- [~] Dart callback 返回值映射为 JS Promise resolve 值：native 已覆盖 JSON-compatible 值。
- [~] Dart callback 抛错映射为 JS Promise reject / JS exception：native 已覆盖。
- [x] native `JS_ExecutePendingJob` job pump。
- [ ] Web quickjs-wasi Promise job pump 对齐。
- [~] `evalAsync` 支持：native 已支持 async 函数体语义；top-level await / Web 待补。
- [ ] dispose runtime 时取消 pending callback 和未完成 Promise。
- [ ] callback runtime 隔离：绑定只在所属 `Quickjs` 实例内有效。
