...
final class NodeFeatures extends JsFeatures {
  factory NodeFeatures({
    ...
    return NodeFeatures._(
      globalBuffer: globalBuffer,
      globalProcess: globalProcess,
      processCoreSource: _nodeProcessCoreSource(
        env: env,
        platform: platform,
        cwd: cwd,
      ),
  ...
  NodeFeatures._({
    ...
    required String processCoreSource,
  }) : super(
         name: 'node',
         scripts: <JsScript>[
           if (globalBuffer)
         ...
         modules: <JsModule>[
           ...
           const JsModule(name: 'crypto', source: _nodeCryptoEsModuleSource),
           const JsModule.commonJs(
             name: 'crypto',
             source: _nodeCryptoCommonJsSource,
           ),
           const JsModule(name: 'path', source: _nodePathEsModuleSource),
           const JsModule.commonJs(
             name: 'path',
           ...
           const JsModule(name: 'timers', source: _nodeTimersEsModuleSource),
           const JsModule.commonJs(
             name: 'timers',
...
const bytesFrom = (value) => {
  if (value instanceof JsBuffer) return Array.from(value);
...
const randomBytes = (size) => new JsBuffer(randomBytesArray(size));
const createHash = (algorithm) => new JsHash(algorithm);
