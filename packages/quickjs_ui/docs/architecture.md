...
2. Page adapter plugin
   ...
   `QuickjsUiPagePlugin` 和 `QuickjsUiBundle.toPlugin()` 把页面模块包装成 QuickJS plugin。现在只暴露 v1 运行时协议：
...
3. Runtime session
   ...
   `QuickjsUiSession` 持有 plugin client、props、Dart 侧 state 快照、当前渲染出的 node tree、生命周期能力和事件队列。权威 state 在 JS 页面运行时中，Dart 只在需要公开 `session.state` 或刷新诊断时同步快照。

4. Controller 和 View

   `QuickjsUiController` 提供 loading/error/node 通知。`QuickjsUiView` 负责加载 asset/file/network/plugin 页面，监听 controller，并把当前 `QuickjsUiNode` 渲染到 Flutter。

5. Renderer

   `QuickjsUiRenderer` 和 `QuickjsUiComponentRegistry` 把 schema node 转成 Flutter widget。内置控件走默认 registry，自定义控件由 Dart 侧注册。
...
路由由 `QuickjsUiNavigator` 和 `QuickjsUiRouteRegistry` 处理。目前有两类 route：

- Native route：注册在 `nativeRoutes` 里的 Flutter builder。
- JavaScript asset route：注册在 `jsRoutes` 里的 quickjs_ui 页面。

导航请求是普通 intent。JS 页面可以发出事件，由宿主把它解释成导航 intent。宿主随后通过 route registry 解析 route 名称、参数和页面来源，再创建对应的 Flutter route 或 `QuickjsUiView.asset(...)`。

Route lifecycle 和普通页面事件分离。`QuickjsUiSession.lifecycle(...)` 和 `routeLifecycle(...)` 使用不同队列，避免页面事件、route 通知和 Flutter frame 回调在同一个调用栈中互相重入。
...
2. `QuickjsUiRenderContext.dispatchEvent(...)` 包装事件。
3. `QuickjsUiEventDispatcher` 应用 widget 级事件策略：command 保序发送，sample 可以按 key 合并，`throttleMs/debounceMs/dropMs` 按策略处理。
4. `QuickjsUiEventIngress` 把事件排到当前 Flutter frame 之后 flush。
5. `QuickjsUiController.dispatch(...)` 调用 `QuickjsUiSession.dispatch(...)`。
6. `QuickjsUiSession` 调用 JS `handleEvent(event)`。
...
页面能力由 `capabilities().lifecycle` 显式声明。`QuickjsUiSession` 在调用 JS 前检查 hook 是否存在，未声明的 hook 不会调用 JS。dispose lifecycle 只发送一次，之后再调用 `dispose()` 释放页面运行时。
...
6. `QuickjsUiEventIngress` 保持 frame-based flush，reentrant event 延后一帧。
...
第一，示例页在 `build()` 中调用 `_customComponentsRegistry()`，每次 Flutter rebuild 都会创建新的 `QuickjsUiComponentRegistry`。窗口大小变化会触发父级 rebuild，`QuickjsUiView.didUpdateWidget()` 看到 registry 引用变化后会 dispose 旧 renderer 并创建新 renderer。这个过程本身不应该调用 JS，但它会和未 flush 的控件事件交错，增加 renderer、event ingress、controller、session 之间的状态切换频率。
...
1. example 页面在 `build()` 中调用 `QuickjsUiVideoPlayerPlugin.registry()`，窗口变化或父级 rebuild 会让 `QuickjsUiView` 看到新的 registry 引用，从而重建 renderer。
...
随后对 `example/assets/quickjs_ui` 和 quickjs_ui 相关测试做了全量清理：所有 handler 中的 `return { ...state, ... }` 和 no-op `return state` 都移除。保留的 `...state.xxx` 只允许用于数组展开或构造校验用的局部对象，不能作为 handler 的返回对象。

`reset` 仍然能够触发 `call=dispatch` 后，说明问题不应继续归因到单个页面方法。后续又做了两处 core 级重构：

1. `QuickjsUiSession` 的操作队列从连续 `.then()` 链改为显式 FIFO drain。高频 dispatch 不再构造越来越长的 Dart Future 链。
