...
- [x] 错误模型
  ...
  - exception model example 已显式展示 `JsException` 的 name、message、stack、fileName、
    line、column。
  - worker crash、OOM 与 native stack overflow 已有稳定错误类型和测试覆盖；Web stack limit
    等待底层暴露等价能力，不阻塞当前版本。

- [x] runtime 隔离
  - globals、dispose、callbacks、timers、module cache、handles 与 resource limits 的隔离已覆盖。
  - Web timeout / stop 触发 worker 重建时 global 状态丢失属于已固定的语义约束。
...
- [x] `evalAsync` 支持：native / web 已支持 async 函数体语义；top-level await 归入后续 ES module 能力评估。
...
- [x] Web Crypto：环境补全 `globalThis.crypto`；`QuickjsWebCryptoMount()`
...
- [x] Node `crypto`：作为 `crypto` / `node:crypto` 模块加载；当前最小子集覆盖
  `randomBytes()` 与 `createHash('sha256')`，支持 ES module 与 CommonJS。HMAC、签名、加密等
  Node 风格 API 后续按需补齐。Web `globalThis.crypto` 与 Node `crypto` 模块是两套入口。
- [x] Node 兼容模块：`process` / `node:process`、`path` / `node:path`、
...
3. [x] 迁移现有预制实现：`crypto.subtle.digest()` 与 `QuickjsFetchMount` 均使用通用 async provider
   注入；后续 Node `crypto` 等能力继续复用同一机制。
4. [x] 环境补全：`QuickjsHostMount.web()` 提供 `URL`、`location`、`navigator`、storage
   等 Web 风格最小集；`QuickjsFetchMount` 作为独立显式能力提供 Fetch API。
5. [x] 批量挂载：已新增 `QuickjsHostMount`、`QuickjsRuntimeOptions.mounts` 和 `Quickjs.mount(...)`，
   ...
   热挂载不纳入当前语义；replace 策略已支持同名 runtime mount 的原子替换。
6. [x] 命名整理：批量入口使用 `mounts` / `QuickjsHostMount`，直接配置统一为
   `environmentPatches`、`modules`、`providers`。
7. [x] `QuickjsWebCryptoMount()`：已提供 `crypto.randomUUID()`、
   ...
   再由用户或独立预制包组合具体能力。
8. [x] `QuickjsHostMount.node()`：已提供 `buffer`、`crypto`、`path`、`process`、`timers` 的
   ES module 与 CommonJS host module；可显式安装 global `Buffer` / `process`。`crypto` 当前为
   `randomBytes()` / `createHash('sha256')` 最小子集；`fs`、网络和完整 npm resolver 不在第一版内。
9. [x] `QuickjsHostMount.essential()`：已提供最小 `buffer` / `node:buffer` ES module 与
   CommonJS module，并可显式安装 global `Buffer`；console 不放在 preset 中，因为 no-op console
   是 core default。timers、JSON/URL 等不作为 essential 当前范围，后续按真实需求进入独立 mount。
10. [x] npm bundle 策略已文档化：完整 npm resolver 不放进 runtime，推荐用户用 esbuild/Rollup
   预打包；`modules` 只负责显式注册的模块。`example/npm_bundle` 提供可构建的 esbuild 工程，
   生成 Flutter asset 并在 native/Web 测试中作为 ES host module 执行；独立 `NPM Bundle` example
   页面只展示加载单文件 asset、注册 module 和调用指定导出方法。

### 验收

- [x] 未启用任何 host environment 时，`fetch`、`window`、`Buffer`、`crypto` 等不可见，或按文档
  返回明确错误。
- [x] global 能力不需要 import：`window.location`、`navigator`、storage、`crypto.randomUUID()`、
  `fetch()` 在启用对应 mount 后可用。
...
- [x] 运行时挂载模块遵守 cache 规则：同名 provider、同格式 module、显式声明的 global，以及
  已由动态 loader 加载的 ES/CommonJS module 都会在重建前报错；显式 replace 只允许替换旧 mount
  自己拥有的 module 名。
...
0.9.0 前的执行模型、模块、provider、mount、Web/Node 最小兼容能力和 example/test 覆盖已收口。
...
- [x] 单文件插件：一个 JS 文件就是一个 entry module，可直接导出业务函数，例如 `hello()`。
- [x] 插件包：多个 JS module 组成一个模块图，包含 entry module、内部依赖模块和可选资源描述。
- [x] 插件模块必须带 namespace，例如 `api1/main`、`api1/xx`，避免不同插件之间 specifier 冲突。
- [x] 插件内部可使用相对 import；框架负责把插件模块图映射到 `modules` 并复用 0.9.0 的
  解析和 cache 规则。
- [x] 插件来源可以是 inline source、asset 或 bytes；file、目录、zip 等来源由外层安装器读取后
  转成 `QuickjsPluginModule`，quickjs 不要求用户把多文件插件预打包成单个 JS 文件。
- [x] 复杂 npm 依赖仍推荐插件作者预打包；quickjs 插件包只承诺显式模块图，不实现完整 npm resolver。

### 插件模板与契约

- [x] 提供推荐模板，而不是强制 JS 继承某个基类。插件默认通过 ES module exports 暴露能力。
- [x] manifest 声明插件契约：`id`、`version`、`entry`、`exports`、可选 `permissions`、可选
  `metadata`。
...
- [x] 插件需要网络、存储、文件系统等能力时，只能通过 0.9.0 host environment 显式启用；
...
目标：探索类似 Xiaomi Vela JSUI / QuickApp 的声明式 UI 能力，但不放进 `quickjs` core。
`quickjs` 核心包只继续提供 runtime、plugin、module、host capability 和 structured value codec；
JSUI 作为独立包（例如 `quickjs_jsui`）依赖这些能力，把 JS 描述的 UI 转成 Flutter 原生 Widget。

### 分包边界

- [ ] 新建独立包/模块承载 JSUI，例如 `packages/quickjs_jsui` 或单独 repo，避免 quickjs core 膨胀。
- [ ] quickjs core 不内置模板解析器、样式系统、Widget renderer、组件库和页面路由。
- [ ] JSUI 包通过 `QuickjsPlugin` / `invokePlugin()` 调用 JS 页面逻辑，通过 `QuickjsHostMount`
  显式启用 fetch、storage、prompt 等宿主能力。
- [ ] JSUI 包只暴露 Flutter Widget 入口，例如 `JsUiView.asset('assets/pages/home.ux')`。

### 第一阶段：最小原型

- [ ] 支持 `.ux` 风格单文件：`<template>`、`<style>`、`<script>` 三段拆分。
- [ ] `<script>` 编译/注册为 `QuickjsPlugin`，支持 `data` 初始化和 method 调用。
- [ ] `<template>` 解析为受限 UI tree，不直接执行任意 DOM/HTML。
- [ ] 支持最小组件映射：`div`、`text`、`button`、`image`、`input`。
- [ ] 支持最小样式映射：`width`、`height`、`padding`、`margin`、`color`、
  `background-color`、`font-size`、`flex-direction`、`align-items`、`justify-content`。
- [ ] 支持 `{{state}}` 文本绑定和基础属性绑定。
- [ ] 支持 `onclick` / `onchange` 调用 JS method，method 返回新 state 或 patch 后 Flutter 重绘。

### 第二阶段：组件与数据流

- [ ] 支持自定义组件 import，但组件树、props 和事件模型由 JSUI 包管理，不进入 quickjs core。
- [ ] 支持父子 props 下发、事件上抛（类似 `$emit` / `$dispatch` 的受限版本）。
- [ ] 支持生命周期：`onInit`、`onReady`、`onDestroy`。
- [ ] 支持列表渲染和条件渲染的最小指令。

### 暂不做

- [-] 不追求完整兼容 Vela / QuickApp / Vue 语法。
- [-] 不实现浏览器 DOM、CSSOM、WebView 或 JS 直接操作 Flutter Widget。
- [-] 不在第一版实现完整 CSS selector、动画、slot、scoped style、复杂 diff。
- [-] 不把系统权限能力默认开放给 JSUI 页面；仍由宿主显式 mount。

## 测试策略

### 当前必须保持通过

开发中使用定向、低输出验证，功能稳定后只运行一次完整门禁：
...
- [~] memory limit / stack limit 测试：memory limit 与 native stack limit 已覆盖；Web stack limit
  等价测试等待底层能力，不阻塞 0.9.0 收口。
- [x] exception model example 补齐结构化 `JsException` 字段展示。
- [x] callback / timer / module cache runtime 隔离测试。
- [-] example app smoke 自动化测试：作为 CI/发布增强项跟进，不作为 0.9.0 前功能准入。
