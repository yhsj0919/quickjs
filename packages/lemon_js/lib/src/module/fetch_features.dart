...
const _fetchMethodName = 'fetch.request';
...
final class FetchFeatures extends JsFeatures {
  ...
  static FetchFeatures _create({
    ...
    final method = JsHostMethod(
      name: _fetchMethodName,
    ...
    required JsHostMethod method,
         ...
         scripts: <JsScript>[
           JsScript(
             ...
             source: _fetchHostScript(_fetchMethodName),
           ),
         ],
         methods: <JsHostMethod>[method],
       );
...
Future<Object?> _sendFetchRequest(
  ...
  if (args.length != 1 || args.single is! Map) {
    throw const JsValueConversionException(
      'QuickJS fetch method expects one request object',
    );
  }
...
String _fetchHostScript(String methodName) {
  final encodedMethodName = jsonEncode(methodName);
  return '''
(() => {
  const method = globalThis.__jsHostMethods[$encodedMethodName];
  ...
  class JsHeaders {
    constructor(init = {}) {
      ...
      if (init instanceof JsHeaders) init = Array.from(init.entries());
  ...
  class JsResponse {
    constructor(payload) {
      ...
      this.headers = new JsHeaders(payload.headers);
  ...
  class JsRequest {
    constructor(input, init = {}) {
      const source = input instanceof JsRequest ? input : null;
      ...
      this.headers = new JsHeaders(init.headers || (source && source.headers) || {});
    ...
    clone() { return new JsRequest(this); }
  }

  const fetch = async (input, init = {}) => {
    const request = new JsRequest(input, init);
  ...
  class JsAbortSignal {
    ...
    static abort(reason) {
      const controller = new JsAbortController();
  ...
  class JsAbortController {
    constructor() { this.signal = new JsAbortSignal(); }
  ...
  class JsXMLHttpRequest {
    constructor() {
      ...
      this._headers = new JsHeaders();
      this._responseHeaders = new JsHeaders();
      this._aborted = false;
      this._sent = false;
    }
    ...
    async send(body = null) {
      ...
      try {
        ...
        this._responseHeaders = new JsHeaders(payload.headers);
  ...
  Object.assign(JsXMLHttpRequest, { UNSENT: 0, OPENED: 1, HEADERS_RECEIVED: 2, LOADING: 3, DONE: 4 });
  Object.assign(JsXMLHttpRequest.prototype, { UNSENT: 0, OPENED: 1, HEADERS_RECEIVED: 2, LOADING: 3, DONE: 4 });

  Object.defineProperties(globalThis, {
    fetch: { value: fetch, configurable: true, enumerable: false, writable: true },
    Headers: { value: JsHeaders, configurable: true, enumerable: false, writable: true },
    Request: { value: JsRequest, configurable: true, enumerable: false, writable: true },
    Response: { value: JsResponse, configurable: true, enumerable: false, writable: true },
    AbortController: { value: JsAbortController, configurable: true, enumerable: false, writable: true },
    AbortSignal: { value: JsAbortSignal, configurable: true, enumerable: false, writable: true },
    XMLHttpRequest: { value: JsXMLHttpRequest, configurable: true, enumerable: false, writable: true },
  });
})();
''';
}
