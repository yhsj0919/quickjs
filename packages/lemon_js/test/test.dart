...
import 'package:lemon_js/src/native/native_worker.dart';
...
void main() {
  ...
  // 同一个 JsEngine 实例应能重复执行，不需要每次重新创建 runtime。
...
''',
      ...
      await expectLater(
        engine.callPluginExport(plugin, 'render', const <Object?>[]),
        throwsA(
          isA<JsValueConversionException>().having(
            (error) => error.message,
            'message',
...
''',
      ...
      await expectLater(
        engine.callPluginExport(plugin, 'echo', <Object?>[value]),
        throwsA(
...
''',
  ...
  test(
    'replacing an injected function removes the previous callback',
    () async {
      final engine = await JsEngine.create();
      addTearDown(engine.dispose);

      await engine.injectFunction('hostValue', (_) => 1);
      await engine.injectFunction('hostValue', (_) => 2);

      expect(await engine.call('hostValue', const <Object?>[]), 2);
    },
  );
...
''');
  ...
  test('eval rejects unsupported Dart globals', () async {
    ...
    await expectLater(
      engine.eval('value', tempGlobals: {'value': Object()}),
      throwsA(isA<JsValueConversionException>()),
    );
    final cyclic = <Object?>[];
    cyclic.add(cyclic);
    await expectLater(
      engine.eval('value', tempGlobals: {'value': cyclic}),
      throwsA(isA<JsValueConversionException>()),
    );
  });
  ...
  // timeout 后 backend 会恢复 runtime，同一个 JsEngine 实例仍可继续执行。
  ...
  test('queued evaluation can time out before it starts', () async {
    ...
    expect(await engine.eval('globalThis.queuedTimeout'), JsUndefined.value);
  });
  ...
  test('stop cancels queued evaluations', () async {
    ...
    await stopFuture;
    expect(await engine.eval('globalThis.stoppedQueue'), JsUndefined.value);
  });

  // 两个 JsEngine 实例的 globalThis 不能互相污染。
  test('multiple runtimes keep globals isolated', () async {
    ...
    expect(await second.eval('globalThis.sharedName'), JsUndefined.value);
  ...
  test('native worker crash completes pending request with error', () async {
    final runtime = await NativeJsWorkerRuntime.create();
  ...
  test('public contexts keep host callbacks isolated', () async {
    ...
    await first.run('globalThis.result = await hostValue(20); return result;');
    await second.run('globalThis.result = await hostValue(20); return result;');
  ...
  test(
    ...
    () async {
      ...
      await disposed.eval('void slowHost()');
  ...
  test('context async evaluation pumps isolated timers', () async {
    ...
    expect(
      await first.run(
        'return await new Promise(resolve => setTimeout(() => resolve(21), 5));',
      ),
      21,
    );
    ...
    expect(
      await second.run(
        'return await new Promise(resolve => setTimeout(() => resolve(40), 5));',
      ),
      40,
    );
  });
  ...
  test('disposing a context cancels only its timers', () async {
    ...
    expect(
      await sibling.run(
        'return await new Promise(resolve => setTimeout(() => resolve(42), 5));',
      ),
      42,
    );
  });
...
''');
  ...
  test(
    ...
    () async {
      ...
      expect(await first.callPluginExport(firstPlugin, 'add', <Object?>[5]), 6);
      expect(
        await second.callPluginExport(secondPlugin, 'add', <Object?>[5]),
        15,
      );
    },
  );
  ...
  test('context loads a plugin in place and preserves globals', () async {
    ...
    addTearDown(runtime.dispose);
    final context = await runtime.createContext();
    await context.eval('globalThis.beforeFeaturesLoad = 41');
    final plugin = JsPlugin.source(
      id: 'dynamic',
      version: '1.0.0',
      source:
          'export function read() { return globalThis.beforeFeaturesLoad + 1; }',
    ...
    expect(
      await context.callPluginExport(plugin, 'read', const <Object?>[]),
      42,
    );
    expect(await context.eval('beforeFeaturesLoad'), 41);
  });
...
Future<void> _expectHundredQueuedEvals(JsEngine engine) async {
