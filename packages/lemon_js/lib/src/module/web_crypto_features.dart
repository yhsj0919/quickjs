...
const _digestMethodName = 'webcrypto.subtle.digest';
const _hmacMethodName = 'webcrypto.subtle.hmac';
...
final class WebCryptoFeatures extends JsFeatures {
  /// 创建可按需启用随机数、摘要与 HMAC 的 Web Crypto 兼容能力。
  WebCryptoFeatures({
         ...
         scripts: <JsScript>[
           JsScript(
             ...
             source: _webCryptoHostScriptSource(
               ...
               digestMethodName: subtleDigest ? _digestMethodName : null,
               hmacMethodName: subtleHmac ? _hmacMethodName : null,
             ),
           ),
         ],
         methods: <JsHostMethod>[
           if (subtleDigest) _digestMethod,
           if (subtleHmac) _hmacMethod,
         ],
       );
}

final JsHostMethod _digestMethod = JsHostMethod(
  name: _digestMethodName,
  debugName: 'host:webcrypto.subtle.digest',
  implementation: JsHostMethodImplementation.dart,
...
final JsHostMethod _hmacMethod = JsHostMethod(
  name: _hmacMethodName,
...
String _webCryptoHostScriptSource({
  ...
  required String? digestMethodName,
  required String? hmacMethodName,
}) {
  ...
  final encodedDigestMethodName = jsonEncode(digestMethodName);
  final encodedHmacMethodName = jsonEncode(hmacMethodName);
  return '''
(() => {
  ...
  if ($installSubtleDigest) {
    ...
    const digestMethodName = $encodedDigestMethodName;
    const digestMethod = methods && digestMethodName ? methods[digestMethodName] : null;
    ...
    Object.defineProperty(subtle, 'digest', {
      value: (algorithm, data) => {
        ...
        return digestMethod(name, bytes).then((result) => {
  ...
  if ($installSubtleHmac) {
    const methods = globalThis.__jsHostMethods;
    const hmacMethodName = $encodedHmacMethodName;
    const hmacMethod = methods && hmacMethodName ? methods[hmacMethodName] : null;
      ...
      value: (format, keyData, algorithm, extractable, keyUsages = []) => {
        ...
        try {
          ...
          return Promise.resolve(Object.freeze({
            ...
            __jsHmacKeyBytes: new Uint8Array(keyBytes),
          }));
        } catch (error) {
    ...
    Object.defineProperty(subtle, 'sign', {
      value: (algorithm, key, data) => {
        ...
        return hmacMethod(hash, key.__jsHmacKeyBytes, bytes).then((result) => {
          const out = result instanceof Uint8Array
            ? result
            : new Uint8Array(result.buffer, result.byteOffset, result.byteLength);
          return out.buffer.slice(out.byteOffset, out.byteOffset + out.byteLength);
