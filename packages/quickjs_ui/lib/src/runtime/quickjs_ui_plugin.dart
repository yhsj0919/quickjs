...
///
/// 第三方 UI 控件通常需要两部分能力：
/// 1. [mounts]：让 JS 能 `import` 对应模块（例如 `quickjs_ui/video_player`）。
/// 2. [configureRegistry]：让 Flutter 渲染层认识 schema 中的 `type`（例如 `VideoPlayer`）。
///
/// 通过 [QuickjsUiPlugin] 把这两部分绑在一起，避免只配置 registry 或只配置 mount
/// 导致页面一半能力缺失。
final class QuickjsUiPlugin {
  /// 创建一个 UI 插件描述对象。
  ///
  /// - [name]：插件稳定名称，用于调试与冲突识别。
  /// - [mounts]：该插件需要的 JS runtime mount 列表。
  ...
  /// 该 UI 插件依赖的 JS runtime mount。
  ///
  /// 会在 [QuickjsUiView] 创建 runtime 时与业务 [QuickjsUiView.mounts] 合并安装。
