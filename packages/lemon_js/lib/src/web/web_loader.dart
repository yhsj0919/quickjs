...
@JS('quickjsNgWeb')
extension type JsWebHost(JSObject _) implements JSObject {
  ...
  external void runtimeRegisterStreamBridge(
    ...
    JSFunction pull,
    JSFunction cancel,
    JSFunction sinkAction,
  );
  external JSPromise<JSAny?> runtimeBindSink(
    JSNumber runtimeId,
    JSNumber sinkId,
    JSString name,
  );
...
@JS('quickjsNgWeb')
external JsWebHost get _webHost;

/// 将 package asset 路径转换成 Flutter Web 运行时可加载的 URL。
String packageAssetUrl(String assetPath) {
  return Uri.base.resolve('assets/packages/lemon_js/$assetPath').toString();
}

/// 注入 [quickjs_web.js] 并返回挂在 `globalThis` 上的 [quickjsNgWeb]。
Future<JsWebHost> loadJsWebHost() async {
  if (_JSObjectStatic.hasOwn(_globalThis, 'quickjsNgWeb'.toJS)) {
    return _webHost;
  }

  // web 插件注册阶段只是预加载；真正 create 时仍要确保脚本已注册全局对象。
  final scriptUrl = packageAssetUrl('assets/web/quickjs_web.js');
