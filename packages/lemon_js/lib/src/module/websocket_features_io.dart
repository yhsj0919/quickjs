...
const _webSocketMethodName = 'websocket.dispatch';

/// 在原生平台提供浏览器兼容 `WebSocket` 全局对象的宿主能力。
final class WebSocketFeatures extends JsFeatures {
  /// 创建 WebSocket 能力并配置来源、连接和消息资源限制。
    ...
    required JsHostMethod method,
         ...
         scripts: <JsScript>[
           JsScript(
             ...
             source: _webSocketHostScript(_webSocketMethodName),
           ),
         ],
         methods: <JsHostMethod>[method],
       );

  /// 允许连接的规范化 WS(S) origin；`null` 表示不限制。
  ...
  /// 此 Features 实例允许同时保持的最大连接数。
  final int maxConnections;

  /// 建立连接时附加的默认 HTTP Header。
  ...
  Future<Object?> dispatch(
    ...
    if (args.length != 1 || args.single is! Map) {
      throw const JsValueConversionException(
        'QuickJS WebSocket method expects one command object',
      );
    }
...
final class _WebSocketConnection {
  ...
  Future<Map<String, Object?>> next(JsHostMethodContext context) {
...
String _webSocketHostScript(String methodName) {
...
(() => {
  ...
  class JsWebSocket {
    ...
    send(data) {
      ...
      method({ op: 'send', id: this._id, data: normalizeBinary(data) })
    ...
    close(code = 1000, reason = '') {
      ...
      if (this._id === null) {
        ...
        this.dispatchEvent(new JsCloseEvent('close', {
          code,
      ...
      method({ op: 'close', id: this._id, code, reason: String(reason) })
        .catch((error) => this._fail(error));
    }
    ...
    async _connect(protocols) {
      try {
        ...
        if (this.readyState !== CONNECTING) {
          await method({ op: 'release', id: payload.id });
          return;
        }
        this._id = payload.id;
    ...
    async _pump() {
      while (this.readyState === OPEN || this.readyState === CLOSING) {
        let event;
        try {
          event = await method({ op: 'next', id: this._id });
        } catch (error) {
          this._fail(error);
          return;
        }
        ...
        if (event.type === 'message') {
          ...
          this.dispatchEvent(new JsMessageEvent('message', { data, target: this }));
    ...
    _release() {
      ...
      this._released = true;
      method({ op: 'release', id: this._id }).catch(() => {});
    }
  }

  Object.assign(JsWebSocket, { CONNECTING, OPEN, CLOSING, CLOSED });
  Object.assign(JsWebSocket.prototype, { CONNECTING, OPEN, CLOSING, CLOSED });
