...
| 包 | 职责 |
|---|---|
| `quickjs` | QuickJS 运行时、模块加载、插件协议、宿主挂载（Host Mount）、结构化值转换 |
| `quickjs_ui` | 页面协议、UI Schema 解析、控件渲染、事件分发、生命周期、导航 |
...
```bash
dart run quickjs_ui:manifest --root assets/quickjs_ui/my_package --id com.example.app --version 1.0.0
```
...
| 资源 | 路径 | 用途 |
|---|---|---|
| JS helper 源码 | `packages/quickjs_ui/js/quickjs_ui.js` | `Page`、`Component`、控件 helper |
| TypeScript 类型 | `packages/quickjs_ui/js/quickjs_ui.d.ts` | 编辑器代码提示与 `checkJs` 校验 |
| JSON Schema | `packages/quickjs_ui/js/quickjs_ui.schema.json` | Schema 校验 |
| 示例页面 | `example/assets/quickjs_ui/` | 各特性演示 |
| 示例 Flutter 页 | `example/lib/pages/` | 宿主集成参考 |
| 示例 jsconfig | `example/assets/quickjs_ui/jsconfig.json` | 编辑器提示配置参考 |

修改 `quickjs_ui.js` 后需重新生成 Dart helper：

```bash
cd packages/quickjs_ui
...
在存放 JS 页面的目录创建 `jsconfig.json`。本仓库示例见 `example/assets/quickjs_ui/jsconfig.json`：
...
```json
{
  "compilerOptions": {
    ...
    "paths": {
      "quickjs_ui": [
        "../../../packages/quickjs_ui/js/quickjs_ui.d.ts"
      ]
    }
  },
  "include": [
...
若通过 pub 引用 `quickjs_ui`，`dart pub get` 后包位于 Pub cache。`jsconfig.json` 仍放在你的 JS 页面根目录，将 `paths` 指到该包内的声明文件，例如：
...
```json
{
  "compilerOptions": {
    ...
    "paths": {
      "quickjs_ui": [
        "../../../../.pub-cache/hosted/pub.dev/quickjs_ui-<version>/js/quickjs_ui.d.ts"
      ]
    }
  },
  "include": ["**/*.mjs"]
...
- 使用 **`path:` 依赖**（团队内 monorepo）时直接相对路径指向 `packages/quickjs_ui/js/quickjs_ui.d.ts`；
...
```bash
dart pub cache path
# 进入该目录下的 hosted/pub.dev/quickjs_ui-<version>/js/quickjs_ui.d.ts
```

#### 10.1.4 第三方 quickjs_ui 插件类型

使用插件子路径 import 时，在 `paths` 中追加映射。例如 `quickjs_ui_video_player`：
  ...
  "compilerOptions": {
    ...
    "paths": {
      ...
      "quickjs_ui/video_player": [
        "../../../packages/quickjs_ui_video_player/js/quickjs_ui_video_player.d.ts"
      ]
    }
  },
  "include": ["**/*.mjs"]
...
```json
{
  "$schema": "../../../packages/quickjs_ui/js/quickjs_ui.schema.json"
}
```
