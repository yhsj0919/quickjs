...
- [x] example 已覆盖 basic eval、async API、runtime worker、queue/reentry、
  runtime isolation、exception model 页面。
...
- [~] `timeout`
  ...
  - web timeout / stop 后 JS global 状态会丢失，需要在文档和测试中保持明确。
...
- [~] runtime 状态机
  - closed / dispose / queue / stop 语义已有测试。
  - 完整 `creating -> ready -> running -> stopping -> closed / failed` 状态模型待补。

- [~] 错误模型
  - timeout、cancel、closed、基础 JS throw 映射已覆盖。
  - JS exception 的 stack/name/file/line/column 仍待结构化。
  - worker crash、OOM、stack overflow 仍需专门测试和错误类型映射。
...
- [ ] worker crash 后 pending Future 必须完成为 error。
- [ ] native 与 web 对同一组 JS 用例的语义保持一致。
...
- [ ] 完整 runtime 状态机：
  `creating -> ready -> running -> stopping -> closed / failed`。
- [ ] failed / crash 状态语义。
- [ ] memory limit：基于 `JS_SetMemoryLimit`。
- [ ] stack limit：基于 `JS_SetMaxStackSize`。
- [ ] handle 所有权规则。
- [ ] dispose / crash / failed 状态测试。
- [ ] web 多 runtime 在 worker terminate 后的恢复策略。
