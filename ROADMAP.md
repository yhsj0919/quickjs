...
- [~] `cancel(requestId)` 只能取消排队任务；正在执行的任务通过 interrupt handler 中断。
  - 当前已实现 timeout 驱动的内部排队取消；`stop()` 可取消当前执行与排队任务并后台重建 runtime；公开 `cancel(requestId)` 待补。
...
- `while (true) {}` 在 timeout 后返回 `JsTimeoutException`。
  - native 使用 `JS_SetInterruptHandler`；web 在无法中断同步 WASM 时 terminate worker，并由 backend 自动重建 runtime。Web 侧 timeout 后同一 `Quickjs` 实例可继续 eval，但 JS 全局状态会丢失。
- `engine.stop()` 能停止正在执行的 eval。
  - 当前 `stop()` 会取消当前 eval 与排队 eval，然后后台重建 runtime；同一 `Quickjs` 实例可继续 eval，但 JS 全局状态会丢失。
...
- Web 当前多个 runtime 仍共享同一个 Worker；普通 eval / dispose 已隔离，但某个 runtime timeout / stop 触发 Worker terminate 后，其他 runtime 会后台恢复为新 runtime，JS 全局状态可能丢失。完整资源边界需要后续改成 runtime 级 worker 或可恢复 runtime registry。

验收：

- `vm1.eval('globalThis.x = 1')` 后，`vm2.eval('globalThis.x')` 返回 `undefined`。
  - 已补 globals 隔离单元测试与 example `Runtime 隔离` 页面。
- `vm1.dispose()` 后，`vm2` 仍可正常 eval。
  - 已补 dispose 隔离单元测试与 example `Runtime 隔离` 页面。
- callback / function handle 不能跨 runtime 调用。
  - callback / function handle 尚未实现，跨 runtime handle 规则待后续 handle 设计时补齐。
...
2. [~] timeout / stop 测试。
   `while (true) {}` 必须可停止。
   - timeout、排队 timeout 取消、stop 取消当前 eval、stop 取消排队 eval 单元测试已补。

3. [~] runtime 隔离测试。
   globals、callbacks、timers、module cache 互不影响。
   - 已补 globals 隔离、dispose 一个 runtime 不影响另一个 runtime；callbacks、timers、module cache 尚未实现。
...
3. [~] native 增加 interrupt handler、timeout、stop。
   - `JS_SetInterruptHandler`、`eval(timeout:)` 与 `stop()` 已补；native `stop()` 通过共享 cancel flag 触发 QuickJS interrupt，随后关闭旧 worker 并后台重建 runtime。
4. [x] web 增加 Web Worker bridge，迁移 WASM QuickJS 到 worker。
5. [~] 建立 runtime 状态机、执行队列、dispose 语义。
   - FIFO 队列、dispose 不悬挂、dispose 取消排队任务、timeout 驱动的内部排队取消、stop 取消并恢复 runtime 已完成；公开 `cancel(requestId)` 与完整状态机待补。
6. [~] 补齐 UI 不阻塞、timeout、runtime 隔离测试。
   - UI 不阻塞、100 次 FIFO、dispose 取消排队任务、timeout、排队 timeout 取消、stop、globals 隔离、dispose 隔离测试已完成；callbacks、timers、module cache 隔离待补。
...
- [~] `timeout`。
  - native interrupt 与 web worker terminate 兜底已实现；Web timeout 后 backend 自动重建 runtime，同一 `Quickjs` 实例可继续 eval，但 JS 全局状态会丢失；仍需在可用 Flutter 工具链上跑通 native/web 自动验证。
- [~] `stop()`。
  - 已实现取消当前 eval 与排队 eval，并后台重建 runtime；公开 `cancel(requestId)` 仍待补。
- [~] `dispose()` 状态机。
  - dispose 后拒绝新请求、eval 中 dispose 不悬挂、取消尚未开始的排队任务已完成；完整状态机待补。
- [~] 基础错误类型。
  - 已补 `JsTimeoutException`、`JsCancelledException`、`JsRuntimeClosedException`；完整 `JsException` / crash / OOM 类型待补。
