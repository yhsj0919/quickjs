...
///
/// A zip plugin package should contain `manifest.json`. The manifest supports the same contract fields as
...
final class JsZipPlugin {
  ...
  static JsPlugin bytes(Uint8List zipBytes, {String? manifestPath}) {
    ...
    return archive(decoded, manifestPath: manifestPath);
  }

  /// Loads a plugin from an already decoded [Archive].
  static JsPlugin archive(Archive archive, {String? manifestPath}) {
    ...
    return JsPlugin(
      ...
      modules: <JsPluginModule>[
        for (final entry in moduleFiles.entries)
          JsPluginModule.bytes(name: entry.key, bytes: entry.value),
      ],
    );
  }
  ...
  static MapEntry<String, Uint8List> _findManifest(
    ...
    if (candidates.isEmpty) {
      throw const JsValueConversionException(
        'JS zip plugin must contain manifest.json',
      );
    }
